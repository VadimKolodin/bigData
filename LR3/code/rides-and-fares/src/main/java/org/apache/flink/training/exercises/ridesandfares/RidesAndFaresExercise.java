...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
        ...
        public void open(Configuration config) {
            rideValueState = getRuntimeContext().getState(new ValueStateDescriptor<TaxiRide>("ride", TaxiRide.class));
            fareValueState = getRuntimeContext().getState(new ValueStateDescriptor<TaxiFare>("fare", TaxiFare.class));
        }

        @Override
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
            if (rideValueState.value() == null && fareValueState.value() == null) {
                rideValueState.update(ride);
            } else {
                out.collect(new RideAndFare(ride, fareValueState.value()));
                fareValueState.update(null);
            }
        }
        ...
        public void flatMap2(TaxiFare fare, Collector<RideAndFare> out) throws Exception {
            if (rideValueState.value() == null && fareValueState.value() == null) {
                ...
                out.collect(new RideAndFare(rideValueState.value(), fare));
                rideValueState.update(null);
            }
        }
    }
}
