...
public class LongRidesExercise {
    ...
    public static class AlertFunction extends KeyedProcessFunction<Long, TaxiRide, Long> {
        ...
        private static final long FIRE_EVENT_TIME_IN_SECONDS = 2 * 3600;

        private ValueState<TaxiRide> rideState;

        @Override
        public void open(Configuration config) throws Exception {
            ValueStateDescriptor<TaxiRide> rideStateDescriptor = new ValueStateDescriptor<>("ride", TaxiRide.class);
        ...
        public void processElement(TaxiRide ride, Context context, Collector<Long> out) throws Exception {
            TaxiRide caughtRide = rideState.value();

            if (caughtRide == null) {
                rideState.update(ride);
                if (ride.isStart) {
                    context.timerService()
                           .registerEventTimeTimer(ride.eventTime.plusSeconds(FIRE_EVENT_TIME_IN_SECONDS).toEpochMilli());
                }
                return;
            }

            if (caughtRide.isStart && shouldAlert(caughtRide, ride) || !caughtRide.isStart && shouldAlert(ride, caughtRide)) {
                alert(out);
            }
        ...
        private void alert(Collector<Long> out) throws Exception {
            out.collect(rideState.value().rideId);
        }
        private void clear(Context context) throws Exception {
            if (rideState.value().isStart) {
                context.timerService()
                       .deleteEventTimeTimer(rideState.value().eventTime.plusSeconds(FIRE_EVENT_TIME_IN_SECONDS).toEpochMilli());

            }
            rideState.clear();
        }

        private boolean shouldAlert(TaxiRide start, TaxiRide end) {
            return start.eventTime.plusSeconds(FIRE_EVENT_TIME_IN_SECONDS).isBefore(end.eventTime);
        }
